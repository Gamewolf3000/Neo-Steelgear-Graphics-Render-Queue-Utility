...
#else
//#include "d3dx12.h"
// We will not be creating actual resources in here, just loading the data
#endif
...
#undef ISBITMASK
    ...
    HRESULT FillInitData(_In_ size_t width,
        ...
        _In_ size_t maxsize,
        _In_ size_t bitSize,
        _In_reads_bytes_(bitSize) uint8_t* bitData,
        _Out_ size_t& twidth,
        ...
        std::vector<void*>& initData)
    {
        ...
        size_t NumBytes = 0;
        size_t RowBytes = 0;
        uint8_t* pEndBits = bitData + bitSize;

        initData.clear();

        for (size_t p = 0; p < numberOfPlanes; ++p)
        {
            uint8_t* pSrcBits = bitData;
            ...
            for (size_t j = 0; j < arraySize; j++)
            {
                ...
                for (size_t i = 0; i < mipCount; i++)
                {
                    ...
                    if ((mipCount <= 1) || !maxsize || (w <= maxsize && h <= maxsize && d <= maxsize))
                    {
                        ...
                        initData.emplace_back(pSrcBits);
                    }
                    else if (!j)
                    {
    ...
    HRESULT CreateTextureFromDDS(
        ...
        CategoryResourceIdentifier& resourceIdentifier,
        ...
        _Out_opt_ bool* outIsCubeMap) noexcept(false)
    {
        ...
        const UINT numberOfPlanes = 1; // We are not using this for planebased formats
        if (!numberOfPlanes)
            return E_INVALIDARG;
        ...
        if (SUCCEEDED(hr))
        {
            ...
            resourceIdentifier = renderer.ResourceCategories().CreateTexture2D(
                categoryIdentifier, width, height, arraySize, mipCount, 1, 0,
                nullptr, cubeView);

            for (std::uint8_t i = 0; i < subresources.size(); ++i)
            {
                renderer.ResourceCategories().SetResourceData(resourceIdentifier, subresources[i], i);
            }
        }
    ...
    bool* isCubeMap)
{
    ...
    HRESULT hr = LoadTextureDataFromFile(fileName,
        ddsData, &header, &bitData, &bitSize);
    ...
    hr = CreateTextureFromDDS(renderer, categoryIdentifier,
        resourceIdentifier, header, bitData, bitSize,
        maxsize, resFlags, loadFlags, isCubeMap);
